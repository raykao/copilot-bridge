...
import type { AppConfig, ChannelConfig, BotConfig, InterAgentConfig, AccessConfig, BridgeProviderConfig, HttpApiKeyConfig } from './types.js';
...
let _resolvedHttpApiKeySecrets = new Map<string, string>();
...
function validateAndNormalize(raw: any): AppConfig {
  const resolvedHttpApiKeySecrets = new Map<string, string>();
  ...
  for (const [name, platform] of Object.entries(raw.platforms)) {
    ...
    if (name === 'http') {
      ...
      if (p.enabled) {
        if (p.apiKeys === null || typeof p.apiKeys !== 'object' || Array.isArray(p.apiKeys) || Object.keys(p.apiKeys).length === 0) {
        ...
        for (const [apiKeyName, apiKey] of Object.entries(p.apiKeys as Record<string, HttpApiKeyConfig>)) {
          ...
          if (typeof keyConfig.secret !== 'string') {
            throw new Error(`Platform "http" apiKey "${apiKeyName}" missing "secret"`);
          }
          ...
          resolvedHttpApiKeySecrets.set(apiKeyName, resolveEnvSecret(keyConfig.secret, `Platform "http" apiKey "${apiKeyName}" secret`));
        }
      }
    } else if (name === 'slack') {
  ...
  _resolvedHttpApiKeySecrets = resolvedHttpApiKeySecrets;
...
export function getHttpApiKeySecret(apiKeyName: string): string | undefined {
  return _resolvedHttpApiKeySecrets.get(apiKeyName);
}
...
export function _resetConfigForTest(): void {
  _config = null;
  _configPath = null;
  _resolvedHttpApiKeySecrets.clear();
